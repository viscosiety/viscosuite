#!/usr/bin/env bash
# send-siu-s12.sh
# SIU^S12 New Appointment Booking — books PAT-9901 for a CARDIO consultation (APT-2026-001).
#
# Env vars:
...
MSG=$(printf '%s\r' \
  "MSH|^~\\&|SchedSystem|HospitalA|viscolink|viscosiety|${TS}||SIU^S12^SIU_S12|${MSG_ID}|P|2.5" \
  "SCH|APT-2026-001^HospitalA|APT-2026-001^HospitalA||||CARDIO-CONS^Cardiology Consultation^L|WALKIN^Walk-in^HL70276||30" \
  "PID|1||PAT-9901^^^HospitalA^MR||van den Berg^Maria^A||19750422|F|||Herengracht 789^^Amsterdam^NH^1017CB^NLD||+31687654321^CP^CP" \
  "NK1|1|van den Berg^Peter^A|SPO^Spouse^HL70063" \
  "PV1|1|O" \
  "RGS|1|A" \
  "AIS|1||CARDIO-CONS^Cardiology Consultation^L|20260610090000||||30" \
