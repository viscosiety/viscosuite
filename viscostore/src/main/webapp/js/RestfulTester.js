...
// Utility to clear previous hidden inputs
function clearFormHiddenInputs() {
	$('#outerForm input[type="hidden"]').remove();
}

// Utility to add hidden input to the form
function addHiddenInput(name, value) {
	$('#outerForm').append($('<input />', { type: 'hidden', name: name, value: value }));
}
...
function updateFromEntriesTable(source, type, id, vid) {
	...
	addHiddenInput('updateId', id);
	var resVid = source.resourcevid;
	if (resVid !== '') {
		//btn.append($('<input />', { type: 'hidden', name: 'updateVid', value: vid }));
		addHiddenInput('updateVid', vid);
	}	
	setResource(btn, type);
...
function executeOperation(source, type, id, operationName) {
   ...
   addHiddenInput('instanceType', type);
   addHiddenInput('instanceId', id);
   addHiddenInput('operationName', operationName);
...
function setResource(target, resourceName) {
	...
	if (resourceName != null) {
		if (resource.length) {
			...
			addHiddenInput('resource', resourceName);
		}
	} else {
		if (resource.length) {
			resource.val('');
