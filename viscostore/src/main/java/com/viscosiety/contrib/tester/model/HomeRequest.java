...
public class HomeRequest {
	...
	private static final List<CachedClient> CLIENT_CACHE = Collections.synchronizedList(new ArrayList<>());

	private static class CachedClient {
		final GenericClient client;
		final long created;

		CachedClient(GenericClient client) {
			this.client = client;
			this.created = System.currentTimeMillis();
		}

		boolean isExpired() {
			return System.currentTimeMillis() - created > 60_000;
		}
	}

	private void purgeExpiredClients() {
		CLIENT_CACHE.removeIf(CachedClient::isExpired);
	}
