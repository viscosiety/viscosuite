#!/usr/bin/env bash
# HTTP-ADT-A04-RegisterPatient.sh — Register a Patient (outpatient visit) over HTTP
# Sends an ADT^A04 message: Jan de Vries (PAT-001) registers for an
# outpatient appointment at the CARDIO outpatient clinic (visit V-2026-002).
#
# Usage:
#   ./HTTP-ADT-A04-RegisterPatient.sh [-h host] [-p port] [--no-validate]
#
