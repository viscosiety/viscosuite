#!/usr/bin/env bash
# HTTP-ADT-A01-AdmitPatient.sh — Admit / Visit Notification over HTTP
# Sends an ADT^A01 message for Jan de Vries (PAT-001, CARDIO) to the
# viscolink HTTP listener. Expects an AA ACK in the response.
#
# Usage:
#   ./HTTP-ADT-A01-AdmitPatient.sh [-h host] [-p port] [--no-validate]
#
